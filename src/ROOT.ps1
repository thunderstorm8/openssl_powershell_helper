...
openssl req -new -config $ca_conf_dir\root\ca.conf -out $ca_home\$ca_name.pem.csr -key $ca_home\private\$ca_name.pem.key
openssl ca -selfsign -config $ca_conf_dir\root\ca.conf -in $ca_home\$ca_name.pem.csr -out $ca_home\certs\$ca_name.pem.crt -extensions root_ca_ext -notext
openssl x509 -outform DER -in $ca_home\certs\$ca_name.pem.crt -out $ca_home\certs\$ca_name.der.crt
openssl ca -gencrl -config $ca_conf_dir\root\ca.conf -out $ca_home\$ca_name.pem.crl
