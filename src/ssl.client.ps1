...
openssl ca -config $CA_CONF_DIR/ca.conf -in $CA_HOME/temp/ssl.client.pem.csr -out $CA_HOME/temp/ssl.client.pem.crt -policy extern_pol -extensions server_ext -notext
$out = openssl x509 -in $CA_HOME/temp/ssl.client.pem.crt -serial -noout
...
openssl pkcs12 -export -name "SSL server certificate" -inkey $OUT_FOLDER/ssl.client.pem.key -in $OUT_FOLDER/ssl.client.pem.crt -CAfile $OUT_FOLDER/ca.chain.pem.crt -out $OUT_FOLDER/ssl.client.full.pfx -password pass:$pass
openssl pkcs12 -export -name "SSL server certificate" -inkey $OUT_FOLDER/ssl.client.pem.key -in $OUT_FOLDER/ssl.client.pem.crt -out $OUT_FOLDER/ssl.client.brief.pfx -password pass:$pass
openssl pkcs12 -in $OUT_FOLDER/ssl.client.full.pfx -out $OUT_FOLDER/ssl.client.full.pem -passin pass:$pass -passout pass:$pass
openssl pkcs12 -in $OUT_FOLDER/ssl.client.brief.pfx -out $OUT_FOLDER/ssl.client.brief.pem -passin pass:$pass -passout pass:$pass
